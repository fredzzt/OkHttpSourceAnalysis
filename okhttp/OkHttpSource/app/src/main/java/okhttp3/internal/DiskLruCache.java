...
//    在DiskLruCache初始化时，将建立线程池，最少零个线程，最大一个线程，线程空闲可以活60s，线程名叫做"OkHttp DiskLruCache"，当JVM退出时，线程自动结束。
   ...
   *  DiskLruCache.Editor对工具类FileSystem进行进一步的封装，它以DiskLruCache.Entry作为构造参数，
   *  通过操控Entry中维护的数组，对外暴露source/sink，为上层的java对象与文件的转换提供基于okio的流操作，
   *  我们可以通过对它的两个方法进行FindUsage查询获得OkHttp关于文件读写的全部场景
  ...
  /**
   * 维护着key对应的多个文件
   * DiskLruCache.Entry针对每个请求的url对应的文件进行引用维护（而没有进行创建/读取等操作），它内部维护了2个File数组，
   * 一般来说每个url对应2～4个文件。 文件名命名规则是{md5(url)+ {0,1}}，后面的0或1，分别表示ENTRY_METADATA与ENTRY_BODY。
