 ...
 * <ol>
 *     <li>It is created.
 *     <li>The HTTP request message is sent with sendRequest(). Once the request is sent it
 ...
 *     <li>The HTTP response message is read with readResponse(). After
 * the response has been read the response headers and body can be read. All responses have a
 * response body input stream, though in some instances this stream is empty.
 * </ol>
 *
...
public final class HttpEngine {
  ...
  public void sendRequest() throws RequestException, RouteException, IOException {
    if (cacheStrategy != null) return; // Already sent.如果已经有缓存的请求策略，那么就直接返回
    ...
    Response cacheCandidate = responseCache != null
        ? responseCache.get(request)//查询responseCache 的key 是否存在
        : null;
...
//      httpStream 实现类 Http2xStream 或者 Http1xStream
      httpStream = connect();
...
//        我们有一个有效的缓存响应。 立即将其提升给用户响应。
  ...
  private HttpStream connect() throws RouteException, RequestException, IOException {
    ...
    return streamAllocation.newStream(client.getConnectTimeout(),
        ...
        client.getRetryOnConnectionFailure(), doExtensiveHealthChecks);//开始建立连接
  }
   ...
   * 在传输之前立即调用传输HTTP请求头。 这是用来观察
       *请求被缓存的发送时间。
   先记录请求发送的时间（被httpStream 调用）
   */
   ...
   * 报告并试图从未能与服务器进行通信的失败中恢复。返回一个新的HTTP
   *如果失败是可恢复的，则应用于重试的引擎，如果是可恢复的，或是无效的
   *永久。只有当身体被缓冲时，一个身体的请求才能恢复。
   */
...
//        内容编码标题（如initcontentstream()）。
   ...
   * 计算响应接收到此引擎的响应的HTTP请求。 这会
       *添加验证标头或跟随重定向。 如果后续是不必要的或
       *不适用，此操作将返回null。
   */
